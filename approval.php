...
switch ($op) {
    ...
    case 'batch_update_booking_pass':
        batch_update_booking_status($item_id, $batch_booking, 1);
        header("location: {$_SERVER['PHP_SELF']}?item_id=$item_id");
        exit;
    //批次不通過
    case 'batch_update_booking_deny':
        batch_update_booking_status($item_id, $batch_booking, 0);
        header("location: {$_SERVER['PHP_SELF']}?item_id=$item_id");
        exit;

    //批次刪除
    case "batch_delete_booking":
        batch_delete_booking($item_id, $batch_booking);
    ...
    default:
        ...
        if (! empty($item_id)) {
            ...
            if ($sections) {
                $booking_data   = Tad_booking_data::get_all(["`section_id` IN($sections)", 'approver' => 0, 'pass_date' => '0000-00-00'], ['week'], [], ['booking_date' => 'ASC', 'section_id' => 'ASC', 'waiting' => 'ASC']);
...
function batch_delete_booking($item_id, $batch_booking_arr)
{
    if (in_array($item_id, $_SESSION['can_approve'])) {

        // 提取所有 booking_id
        $booking_id_arr = array_merge(...array_map('array_keys', $batch_booking_arr));
        $all_booking    = Tad_booking::get_all(['`id` IN(' . implode(',', $booking_id_arr) . ')'], [], [], [], 'id');

        foreach ($batch_booking_arr as $section_id => $section_booking) {
            foreach ($section_booking as $booking_id => $booking_date) {
...
// 審核通過
function batch_update_booking_status($item_id, $batch_booking_arr, $status)
{
    if (in_array($item_id, $_SESSION['can_approve'])) {
        $item = Tad_booking_item::get(['id' => $item_id]);
        // 提取所有 section_id
        $section_id_arr = array_keys($batch_booking_arr);
        ...
        redirect_header($_SERVER['PHP_SELF'], 3, _MD_TADBOOKING_CANT_REVIEW);
    }
