<script type='text/javascript'>
    function update_booking_status(booking_id, booking_date, section_id, item_id, status){
        $.post("ajax.php", {op: 'update_booking_status', booking_id: booking_id, booking_date: booking_date, section_id: section_id, item_id: item_id ,status:status},
        function(data) {
            if(data=='1'){
...
<form action="approval.php" method="post" id="myForm" class="form-horizontal">
    ...
    <{if $booking_data}>
        <table data-toggle="table" data-pagination="false" data-mobile-responsive="true" class="mb-3">
            <thead>
                <th class="c" data-sortable="true">#</th>
                <th class="c" data-sortable="true"><{$smarty.const._MD_TADBOOKING_ITEM_TITLE}></th>
            ...
            <{foreach from=$booking_data item=data name=booking}>
                <{assign var="booking_id" value=$data.booking_id}>
                <{assign var="section_id" value=$data.section_id}>
                <{assign var="item_id" value=$data.item_id}>
                <{if $items|default:false}>
                    <{assign var="item" value=$items.$item_id}>
                <{/if}>
                <tr class="<{$booking_id}>_<{$data.booking_date}>_<{$section_id}>">
                    ...
                    <td class="c">
                        <a href="approval.php?item_id=<{$item_id}>"><{$item.title}></a>
                    </td>
                    <td class="c">
                        <input type="checkbox" name="batch_booking[<{$item_id}>][<{$section_id}>][<{$booking_id}>][]" value="<{$data.booking_date}>" <{if $data.waiting==1}>checked<{/if}>>
